...
1. **项目开始时**：立刻创建主文件 `<slug>.md`（内容可不完整）、`ux-notes.md`（模板占位）、`constraints.md`（模板占位，后续主动采集填充）。
2. **每条 User Story 聊清楚后**：立刻写对应的 `user-stories/us-NNN-<name>.md`，并更新主文件索引。
3. **每个 Feature 聊清楚后**：立刻写对应的 `features/feat-NNN-<name>.md`（含验收标准），并更新主文件索引。
4. **主文件随时更新**：每次有实质内容变化就更新 `<slug>.md`。
5. **UX 说明随时采集**：对话中遇到界面风格、页面需求、操作流程、平台要求等信息时，立刻更新 `ux-notes.md`。引导用户用自己的语言描述（如"类似飞书的风格"、"客服打开工单然后 AI 推荐回复"），原型 agent 会据此生成原型。
6. **业务约束随时采集**：在对话中遇到安全、性能、数据集成、访问范围、业务规则等信息时，立刻更新 `constraints.md`。采集时用用户自己的语言——不需要技术表达，描述他们能感受到的要求即可（如"高峰期 1000 人同时访问，响应时间小于 2 秒"、"需要满足个人信息保护法"、"员工数据从 HR 系统拿"）。在核心 features 聊清楚之后，若 constraints 仍有空白项，主动追问。
6. **续聊时**：先读主文件了解当前进度，复述已完成项和待补项，再继续。
